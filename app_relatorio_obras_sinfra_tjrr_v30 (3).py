    ...
    with closing(get_db()) as conn:
        with conn:
            ...
            if not cursor.fetchone():
                ...
                salt_f1 = secrets.token_hex(16)
                hash_f1 = hashlib.pbkdf2_hmac('sha256', 'Obras2026#camila'.encode(), salt_f1.encode(), 100000).hex()
                cursor.execute("""
                INSERT INTO usuarios (login, nome, email, senha_hash, salt, perfil, cargo)
                VALUES ('fiscal.camila', 'Camila Maria Almeida de Carvalho', 'camila.carvalho@tjrr.jus.br', ?, ?, 'fiscal', 'Engenheira Fiscal de Obras')
                """, (hash_f1, salt_f1))

                salt_f2 = secrets.token_hex(16)
                hash_f2 = hashlib.pbkdf2_hmac('sha256', 'Torres2026#tjrr'.encode(), salt_f2.encode(), 100000).hex()
                cursor.execute("""
                INSERT INTO usuarios (login, nome, email, senha_hash, salt, perfil, cargo)
                VALUES ('Derick Torres', 'Derick Gabriel Pereira Torres', 'dekatrrs@gmail.com', ?, ?, 'fiscal', 'Engenheiro Fiscal de Obras')
                """, (hash_f2, salt_f2))

                salt_v = secrets.token_hex(16)
                hash_v = hashlib.pbkdf2_hmac('sha256', 'Leitor2026#tjrr'.encode(), salt_v.encode(), 100000).hex()
                cursor.execute("""
                INSERT INTO usuarios (login, nome, email, senha_hash, salt, perfil, cargo)
                VALUES ('visitante.sinfra', 'Visitante / Consulta Institucional', 'visitante@tjrr.jus.br', ?, ?, 'visualizador', 'Visualizador SINFRA')
                """, (hash_v, salt_v))
                ...
                VALUES (?, ?, ?, ?, ?, ?, 2, 'Camila Maria Almeida de Carvalho', '2026-07-01', '2027-08-16', ?, ?)
                """, (
...
def generate_pdf_bytes(obra_dict, atividades, historico):
    ...
    pdf.set_text_color(*COR_NAVY)
    pdf.cell(186, 4, obra_dict.get('fiscal_nome', 'Camila Maria Almeida de Carvalho'), align="C", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
...
<title>TJRR - Relatório Semanal de Obras (SINFRA)</title>
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; }}
  body {{ min-height: 100vh; height: 100vh; background: #0A192F url('https://images.unsplash.com/photo-1541888946425-d0fbb186a5b3?auto=format&fit=crop&w=1920&q=80') no-repeat center center; background-size: cover; position: relative; display: flex; justify-content: flex-end; align-items: center; overflow: hidden; }}
  .overlay {{ position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(135deg, rgba(10, 25, 47, 0.88) 0%, rgba(15, 76, 129, 0.82) 50%, rgba(2, 132, 199, 0.68) 100%); z-index: 1; }}

  /* LOGO SINFRA LATERAL ESQUERDA - MARCA D'ÁGUA EM 2 LINHAS */
  .sinfra-watermark {{ position: absolute; left: 3vw; top: 50%; transform: translateY(-50%); z-index: 2; color: rgba(255, 255, 255, 0.88); font-family: 'Impact', 'Arial Black', 'Bebas Neue', sans-serif; font-size: 36vh; font-weight: 900; line-height: 0.76; letter-spacing: -4px; text-transform: uppercase; user-select: none; pointer-events: none; }}
  .sinfra-watermark span {{ display: block; }}

  .login-card {{ position: relative; z-index: 3; width: 100%; max-width: 480px; background: #FFFFFF; border-radius: 20px; padding: 40px; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.45); margin-right: 8vw; }}
  .card-header {{ text-align: center; margin-bottom: 24px; }}
  .institution-tag {{ font-size: 11px; font-weight: 800; color: #0284C7; letter-spacing: 1.2px; text-transform: uppercase; background: #F0F9FF; display: inline-block; padding: 6px 14px; border-radius: 20px; border: 1px solid #BAE6FD; }}
  .app-title {{ font-size: 21px; font-weight: 800; color: #0F172A; margin-top: 10px; letter-spacing: -0.5px; }}

  .tabs-nav {{ display: flex; border-bottom: 2px solid #F1F5F9; margin-bottom: 24px; }}
  .tab-btn {{ flex: 1; text-align: center; padding: 10px 2px; font-size: 11px; font-weight: 700; color: #64748B; text-transform: uppercase; letter-spacing: 0.5px; cursor: pointer; border-bottom: 3px solid transparent; transition: all 0.2s; }}
  ...
  .form-input {{ width: 100%; height: 46px; padding: 10px 16px; font-size: 13.5px; color: #0F172A; background: #F8FAFC; border: 1.5px solid #CBD5E1; border-radius: 8px; outline: none; transition: all 0.2s; }}
  .form-input:focus {{ border-color: #0284C7; background: #FFFFFF; box-shadow: 0 0 0 3px rgba(2, 132, 199, 0.15); }}

  .btn-code {{ float: right; height: 32px; padding: 0 14px; background: #E0F2FE; color: #0369A1; font-size: 11px; font-weight: 800; border: 1px solid #BAE6FD; border-radius: 6px; margin-top: 4px; cursor: pointer; }}
  .btn-code:hover {{ background: #BAE6FD; }}

  .btn-primary {{ width: 100%; height: 48px; background: linear-gradient(135deg, #0284C7 0%, #0369A1 100%); color: #FFFFFF; font-size: 13.5px; font-weight: 800; letter-spacing: 0.5px; text-transform: uppercase; border: none; border-radius: 8px; cursor: pointer; margin-top: 8px; box-shadow: 0 4px 14px rgba(2, 132, 199, 0.35); transition: all 0.2s; }}
  .btn-primary:hover {{ transform: translateY(-1px); box-shadow: 0 6px 18px rgba(2, 132, 199, 0.45); }}

  .card-footer {{ text-align: center; margin-top: 20px; font-size: 11px; color: #94A3B8; }}
  ...
  @media (max-width: 1100px) {{ .sinfra-watermark {{ font-size: 26vh; left: 1vw; }} }}
  @media (max-width: 850px) {{ body {{ justify-content: center; padding: 20px; }} .sinfra-watermark {{ display: none; }} .login-card {{ margin-right: 0; }} }}
</style>
</head>
<body>
...
<div class="login-card">
  ...
  <div class="tabs-nav">
    ...
    <div class="tab-btn {TAB_ESQUECI_ACTIVE}" onclick="switchTab('esqueci-senha')">ESQUECI A SENHA</div>
  </div>
  ...
  <!-- Aba 3: ESQUECI A SENHA (OTP) -->
  <div id="tab-esqueci-senha" class="tab-content {TAB_ESQUECI_ACTIVE}">
    <form action="/gerar_codigo" method="POST" style="margin-bottom: 12px;">
      <div class="form-group">
        <label class="form-label">E-mail Cadastrado</label>
        <input type="email" name="email" class="form-input" value="{PREFILL_EMAIL}" placeholder="Digite seu e-mail cadastrado" required>
        <button type="submit" class="btn-code">Gerar Código OTP</button>
    ...
    <form action="/redefinir_senha" method="POST">
      ...
      <div class="form-group">
        <label class="form-label">Código de Verificação (OTP)</label>
        <input type="text" name="codigo" class="form-input" placeholder="Digite o código de 6 dígitos" required>
      </div>
      <div class="form-group">
        <label class="form-label">Nova Senha</label>
        <input type="password" name="nova_senha" class="form-input" placeholder="Digite a nova senha" required>
      </div>
      <button type="submit" class="btn-primary">REDEFINIR MINHA SENHA</button>
    </form>
  </div>
...
<script>
  function switchTab(tabName) {{
    ...
    }} else if (tabName === 'esqueci-senha') {{
      document.querySelectorAll('.tab-btn')[2].classList.add('active');
      document.getElementById('tab-esqueci-senha').classList.add('active');
    }}
  }}
</script>
...
class SinfraHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
        ...
        if path == "/dashboard" or path == "/":
            ...
            if not user:
                ...
                login_html = LOGIN_HTML_TEMPLATE.format(
                    ...
                    TAB_ESQUECI_ACTIVE=tab_esqueci_active,
                    PREFILL_EMAIL=esc(email_req)
                )
            ...
            if is_admin:
                sidebar_nav_menu_html += f'<li><button class="nav-item-btn {nav_adm_act}" onclick="switchModule(\'admin\', this)"><span class="nav-icon">⚙️</span> Gestão de Usuários & Obras</button></li>'
            ...
            if relatorios_salvos:
                ...
                <tr>
                  ...
                  <td>Semana Atual</td>
                  <td>Camila Maria Almeida de Carvalho</td>
                  <td><span class="badge-active">Arquivado</span></td>
    ...
    def do_POST(self):
        ...
        if self.path == "/cadastrar_obra":
            ...
            f_u = cursor.fetchone()
            fiscal_nome = f_u['nome'] if f_u else 'Camila Maria Almeida de Carvalho'
        ...
        # 8. ROTA GERAR CODIGO OTP
        if self.path == "/gerar_codigo":
            email = params.get('email', [''])[0].strip()
            conn = get_db()
            cursor = conn.cursor()
            ...
            if u_row:
                codigo_otp = str(secrets.randbelow(899999) + 100000)
                cursor.execute("INSERT INTO recuperacao_senha (email, codigo) VALUES (?, ?)", (email, codigo_otp))
                conn.commit()
                conn.close()

                self.send_response(302)
                self.send_header('Location', f'/?msg=' + urllib.parse.quote(f'Código OTP gerado com sucesso! Código (Simulação): {codigo_otp}') + f'&tab=esqueci-senha&email={urllib.parse.quote(email)}')
                self.end_headers()
                return
            else:
                conn.close()
                self.send_response(302)
                self.send_header('Location', '/?msg=' + urllib.parse.quote('Erro: E-mail não encontrado ou inativo.') + '&tab=esqueci-senha')
                self.end_headers()
                return

        # 9. ROTA REDEFINIR SENHA COM OTP
        if self.path == "/redefinir_senha":
            email = params.get('email', [''])[0].strip()
            ...
            conn = get_db()
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM recuperacao_senha WHERE email = ? AND codigo = ? AND usado = 0 ORDER BY id DESC LIMIT 1", (email, codigo))
            ...
            if otp_row:
                ...
                cursor.execute("UPDATE usuarios SET senha_hash = ?, salt = ? WHERE email = ?", (senha_hash, salt, email))
                cursor.execute("UPDATE recuperacao_senha SET usado = 1 WHERE id = ?", (otp_row['id'],))
                conn.commit()
                conn.close()

                self.send_response(302)
                self.send_header('Location', '/?msg=' + urllib.parse.quote('Senha redefinida com sucesso! Faça seu login com a nova senha.') + '&tab=login')
                self.end_headers()
                return
            else:
                conn.close()
                self.send_response(302)
                self.send_header('Location', f'/?msg=' + urllib.parse.quote('Erro: Código OTP de verificação inválido para este e-mail.') + f'&tab=esqueci-senha&email={urllib.parse.quote(email)}')
                self.end_headers()
                return
...
def start_server():
    for port in DEFAULT_PORTS:
        try:
            httpd = ReusableTCPServer(("127.0.0.1", port), SinfraHTTPRequestHandler)
            print(f"Servidor Integrado SINFRA / TJRR (v30) rodando em http://localhost:{port}")
    ...
    httpd = ReusableTCPServer(("127.0.0.1", 0), SinfraHTTPRequestHandler)
    assigned_port = httpd.socket.getsockname()[1]
    print(f"Servidor Integrado SINFRA / TJRR (v30) rodando em http://localhost:{assigned_port}")
    threading.Timer(1.2, lambda: webbrowser.open(f"http://localhost:{assigned_port}")).start()
    httpd.serve_forever()
